...
async def simprate(self, ctx, member: discord.Member = None):
    if member == None:
        ...
        for i in range(100):
            ...
            description=f"{member.name} is {valuee} Simp.",
        )
...
async def uselessweb(ctx):
    ...
    await ctx.send(f"{random.choice(lonks)}")
...
@commands.command()
async def hack(ctx, member: discord.Member, message=None):

    logins = [
        f"**Login:**\n**Email:** `{member.name}****@dad.com`\n**Password:** `sweet potatoes`",
...
async def kill(ctx, member: discord.Member):
    ...
    deaths = [
        f"{ctx.message.author}"[:-5] + f" shoots {member.name} in the head.",
        f"{member.name} dies due to Depression.",
        f"{ctx.message.author}"[:-5]
        + f" drags {member.name}'s ears too hard and rips them off.",
        f"{ctx.message.author}"[:-5]
        + f" killed {member.name} by ripping the skin off of their face and making a mask out of it.",
        f"{ctx.message.author}"[:-5] + f" ripped {member.name}'s Heart.",
        f"{ctx.message.author}"[:-5] + f" Alt+F4'd {member.name}.exe!",
        f"{ctx.message.author}"[:-5] + f" swallowed {member.name}.",
    ]
